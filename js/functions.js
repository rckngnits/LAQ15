...
$(function(){
	...
	$('.item_service1').mouseenter(function(e) {
		...
		$(this).children('audio').stop().animate({volume:1},500);
	});
	$('.item_service1').mouseleave(function(e) {
		$(this).children('audio').stop().animate({volume:0},800);
		//$(this).children('audio')[0].currentTime = 0;
	});
    ...
    $('.content_head a, .audio_button').mouseenter(function(e) {
		...
		$(this).children('audio')[0].currentTime = 0;
		$(this).children('audio')[0].play();
		$(this).children('audio').stop().animate({volume:1},0);
	});
		...
		$('html,body').mousemove(function(xy) {
		   ...
		   /* $('.text_shadow').each(function(index, element) {
			...
			for (var i = 0; i < $boxShadow.length;  i++) {
				...
				TweenMax.to($boxShadow[i], 0.5, {css: {left: -thisPLeft*0.02, top:-thisPosCenter*0.02 }});
			};
			
			
			TweenMax.to($(".bg1"), .7, {css: {left: -((middle2*0.015))}});
			TweenMax.to($(".cuadros1"), .7, {css: {left: -(middle2*0.035)}});
			TweenMax.to($(".cuadros2"), .7, {css: {left: -(middle2*0.025)}});
			TweenMax.to($('.content_path1, .content_path2, .parallax1'), .7, {css: {x: -(middle2*0.035)}});
			TweenMax.to($('.parallax2'), .7, {css: {x: -(middle2*0.025)}});
		});
...
if (!(navigator.appVersion.indexOf("Mac")!=-1)){
	...
	$(window).mousewheel(function(event, delta) {
		if (delta < 0){
			$('html,body').stop().animate({scrollTop:'+=140'},800,'easeOutCubic');
		} else{
			$('html,body').stop().animate({scrollTop:'-=140'},800, 'easeOutCubic');
		}
		return false;
	...
	$('.popup').mousewheel(function(event, delta) {
		...
		if (delta < 0){
			$('.popup').stop().animate({scrollTop:'+=140'},800,'easeOutCubic');
		} else{
			$('.popup').stop().animate({scrollTop:'-=140'},800, 'easeOutCubic');
		}
		return false;
	...
	$('.contact').mousewheel(function(event, delta) {
		event.stopPropagation();
		if (delta < 0){
			$('.contact').stop().animate({scrollTop:'+=140'},800,'easeOutCubic');
		} else{
			$('.contact').stop().animate({scrollTop:'-=140'},800, 'easeOutCubic');
		}
		return false;
...
$(window).scroll(function(e) {
	TweenMax.to($('.box-shadow'), 0, {css: {left: 0, top: 0 }});
	TweenMax.to($('.text_shadow'), 0, {css:{textShadow:'0 0 0 rgba(24,230,152,0.5)'}});
...
$(window).load(function(e){
	...
	setTimeout(function(){
		...
		setTimeout(function(){
			$('.content_1').css('transform','translateY(0)').animate({opacity:1},2000, 'easeOutCubic');
		},700);
		setTimeout(function(){
			$('.points1').animate({opacity:1},1000);
		},2000);
	},800);
});
...
$(window).bind('load resize',function(){
	...
	if(winWidth>600){
		...
		$('.about, .services').height('auto').css('margin-bottom', 100);
	}
...
$(window).bind('load resize scroll',function(){
	...
	$('.list_menu_resp a.anchor_resp').each(function(index, element) {
		...
		$(this).click(function(e) {
			...
			menu=0
			$('html,body').stop().animate({scrollTop:hrefPos-100},3000,'easeInOutQuart');
		});
	});
	...
	if( isMobile.any() ) {
		...
		$('.cuadros2').stop().css({top:valScroll*0.45});
		$('.bg1').stop().css({top:valScroll*0.75});
		...
		$('.content_head .anchor').each(function(index, element) {
			...
			$(this).click(function(e) {
				cancelEvent(e);
				$('html,body').stop().animate({scrollTop:hrefPos-140},3000,'easeInOutQuart');
			});
		});
	}
});
...
/*----funcion del slider de Audiofolio--*/
$(function(){
	...
	function disabled(){
		$('.item_exp').bind('mouseenter mouseleave',function(e) {
			if($(this).hasClass('disabled')){
				$(this).children('audio')[0].pause();
				$(this).children('audio')[0].currentTime = 0;
				$(this).children('audio').stop().animate({volume:0},0);
			}
		});
	}
	...
	$(function(){
		...
		$('.item_chapter').mouseenter(function(e) {
			if($(this).hasClass('active')){
				...
				$(this).children('audio')[0].currentTime = 0;
				$(this).children('audio')[0].play();
				$(this).children('audio').stop().animate({volume:1},700);
		...
		$('.main_chapter2').mouseenter(function(e) {
			if($(this).hasClass('active')){
				$('.chapter_1').find('audio').stop().animate({volume:0},1500);
				$(this).children('audio')[0].play();
				$(this).children('audio').stop().animate({volume:1},500);
		...
		$('.main_chapter3').mouseenter(function(e) {
			if($(this).hasClass('active')){
				$('.chapter_2').find('audio').stop().animate({volume:0},2000);
				$(this).children('audio')[0].play();
				$(this).children('audio').stop().animate({volume:1},1500);
		...
		$('.item_final').mouseenter(function(e) {
			var $this = $(this);
			if($(this).hasClass('active')){
				$('.chapter_3').find('audio').stop().animate({volume:0},2000);
				setTimeout(function(){
					$this.children('audio')[0].play();
					$this.children('audio').stop().animate({volume:1},1500);
				},1000);
...
function backToHome(){
	$('.popup').fadeOut(300);
	$('.content_experience').animate({opacity:0},1000);
	setTimeout(function(){
		$('.content_experience').hide().empty();
		$('.main_content').show().animate({opacity:1},1000);
	},1000);
